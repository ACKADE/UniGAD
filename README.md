...
> 本指南专为完全零基础的本科生编写，将一步一步手把手教你如何在自己的电脑上运行 UniGAD 项目。**不需要任何提前知识**，照着做就行！
...
4. [第三步：打开"命令行"（终端/命令提示符）](#4-第三步打开命令行终端命令提示符)
...
| 要求 | 说明 |
|------|------|
| 操作系统 | Windows 10/11、macOS 12+、或 Ubuntu 20.04+ Linux |
...
Anaconda 是一个帮你管理 Python 和各种科学库的工具，**强烈推荐**新手使用。
...
## 4. 第三步：打开"命令行"（终端/命令提示符）

**命令行**是我们后续所有操作的主战场。

### Windows 用户（推荐用 Anaconda Prompt）

1. 点击 Windows 开始菜单
2. 搜索 **"Anaconda Prompt"**
3. 点击打开，你会看到一个黑色窗口，里面有 `(base)` 字样

> **提示：** 也可以搜索并打开 **"命令提示符"（cmd）**，但 Anaconda Prompt 对本项目更友好。

### macOS 用户

1. 按下 `Command（⌘）+ 空格键`，打开 Spotlight 搜索
2. 输入 **"Terminal"**（终端），按回车打开
...
> 执行后你会进入项目的根目录。你可以用 `ls`（macOS/Linux）或 `dir`（Windows）查看里面的文件。
...
> **Windows 用户提示：** 如果提示"目录已存在"，忽略该提示即可。
...
**解决：** 重新安装 Anaconda，安装时一定要勾选 **"Add to PATH"** 选项。Windows 用户可以搜索并打开 **"Anaconda Prompt"** 而不是普通的命令提示符。
...
**原因：** GPU 显存不够。  
**解决：** 减小 `batch_size` 参数，例如将 `--batch_size 1` 改为 `--batch_size 1`（已是最小值），或者改用 CPU 运行（删除 CUDA 相关设置，程序会自动用 CPU）。
...
```
1. 打开命令行（Anaconda Prompt 或 Terminal）
...
- 查看项目原始说明文档：[README.MD](./README.MD)
